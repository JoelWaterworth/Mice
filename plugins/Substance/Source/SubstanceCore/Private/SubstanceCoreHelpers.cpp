...
TArray<SubstanceAir::GraphInstance*> LoadingQueue;

//Queue used to limit substances being rebuilt that will be loaded before LoadingQueue;
TArray<SubstanceAir::GraphInstance*> PriorityLoadingQueue;

//Queue used by RenderAsync
TArray<SubstanceAir::GraphInstance*> AsyncQueue;

//Queue used by RenderAsync
TArray<SubstanceAir::GraphInstance*> CurrentRenderSet;
...
void RemoveFromDelayedImageInputs(SubstanceAir::InputInstanceImage* ImgInput)
{
	...
	TArray<SubstanceAir::GraphInstance*>* CurrentLoadingQueue = &LoadingQueue;
...
void SetDelayedImageInput()
{
	...
	for (const auto& InputItr : DelayedImageInputs)
	{
		...
		SubstanceAir::GraphInstance* Instance = InputItr.second;
...
void CreateTextures(USubstanceGraphInstance* Graph)
{
	...
	//#TODO:: [UE4-384] Add translucent material support
...
void CreateTransientTextures(USubstanceGraphInstance* Graph, UObject* WorldContextObject)
{
	...
	for (uint32 Idx = 0; Idx < Graph->Instance->getOutputs().size(); ++Idx)
	{
		...
		if (bCreateAllOutputs || isSupportedByDefaultShader(Output))
		{
			CreateSubstanceTexture2D(Output, true, FString(Desc->mChannelStr.c_str()), Outer, Graph);
		}
	}

	if (!hasRoughness && GlossinessOutput && !bCreateAllOutputs)
	{
		CreateSubstanceTexture2D(GlossinessOutput, true, FString(GlossinessOutput->mDesc.mChannelStr.c_str()), Outer, Graph);
	}

	if (!hasBaseColor && DiffuseOutput && !bCreateAllOutputs)
	{
		CreateSubstanceTexture2D(DiffuseOutput, true, FString(DiffuseOutput->mDesc.mChannelStr.c_str()), Outer, Graph);
	}
}
...
void CreateSubstanceTexture2D(SubstanceAir::OutputInstance* Output, bool Transient, FString Name, UObject* InOuter, USubstanceGraphInstance* ParentInst)
{
	...
	FName uName = MakeUniqueObjectName(InOuter, USubstanceTexture2D::StaticClass(), *Name);
	Texture = NewObject<USubstanceTexture2D>(InOuter, uName, Transient ? RF_NoFlags : RF_Public | RF_Standalone);
	...
	case PF_G8:
		Texture->SRGB = false;
		check(Output->mDesc.mChannel != SubstanceAir::Channel_Normal);
		break;
	case PF_G16:
		Texture->SRGB = false;
		break;
...
void ValidateFormat(USubstanceTexture2D* Texture, SubstanceAir::OutputInstance* Output)
{
	...
	if (Desc->mChannel == SubstanceAir::ChannelUse::Channel_Normal && Desc->mFormat != Substance_PF_RAW)
	{
		NewFormat = Substance_PF_BC5;
	}
...
const SubstanceAir::GraphDesc* FindParentGraph(const std::vector<SubstanceAir::GraphDesc>& Graphs, SubstanceAir::GraphInstance* Instance)
{
...
void ResetGraphInstanceImageInputs(USubstanceGraphInstance* GraphInstance)
{
	...
	{
		return (CurElement.second == GraphInstance->Instance);
	});
}
...
void PerformDelayedDeletion()
{
	...
	{
		for (const auto& TextureItr : local::TexturesToDelete)
		{
			CastChecked<USubstanceTexture2D>(TextureItr)->ParentInstance->MarkPackageDirty();
			TextureItr->MarkPackageDirty();
		}

		bDeletedSomething = (ObjectTools::ForceDeleteObjects(local::TexturesToDelete, false) != 0) || bDeletedSomething;
	...
	for (const auto& GraphInstance : local::InstancesToDelete)
	{
		...
		{
			//NOTE:: If the parent factory for this graph was deleted prior to this call
			//the instance and all of it's outputs will no longer be valid meaning we have
			//no way to access to textures to add them to be cleaned up. It may be wise to do
			//what we did for the factory and register the associated outputs with the graph instance.
			//This adds only a small amount of overhead and prevents issues like this in the future. For now - KS
			if (OutputItr->mUserData != 0)
			{
		...
		{
			//Otherwise, delete the instance if needed
			local::TexturesToDelete.Empty();
	...
	//Clean up remaining graph instance factories
...
int32 UpdateInput(SubstanceAir::GraphInstance* Instance, const SubstanceAir::InputInstanceBase* InputInstance, class UObject* InValue)
{
	int32 ModifiedOuputs = 0;
...
int32 UpdateInput(SubstanceAir::GraphInstance* Instance, const SubstanceAir::InputInstanceBase* InputInstance, const TArray<int32>& InValue)
{
	return UpdateInputHelper(Instance, InputInstance->mDesc.mUid);
}

int32 UpdateInput(SubstanceAir::GraphInstance* Instance, const SubstanceAir::InputInstanceBase* InputInstance, const TArray<float>& InValue)
{
	return UpdateInputHelper(Instance, InputInstance->mDesc.mUid);
}

int32 UpdateInput(SubstanceAir::GraphInstance* Instance, const SubstanceAir::InputInstanceBase* InputInstance, const FString& InValue)
{
	return UpdateInputHelper(Instance, InputInstance->mDesc.mUid);
}

int32 UpdateInputHelper(SubstanceAir::GraphInstance* Instance, const uint32& Uid)
{
...
//Used by the editor
SubstanceAir::PackageDesc* InstantiatePackage(void* archiveData, uint32 size)
{
	SubstanceAir::PackageDesc* NewPackDesc = AIR_NEW(SubstanceAir::PackageDesc)(archiveData, size);
	return NewPackDesc;
}
	...
	for (const auto& MatItr : Materials)
	{
		for (const auto& OutItr : Graph->Instance->getOutputs())
		{
			...
			if (MatItr.ParameterNames.Contains(OutItr->mDesc.mIdentifier.c_str()) && OutItr->mUserData)
			{
				...
				//#TODO:: This could be dangerous if the user has manually changed the parameter name. We may need to save out a map of parameter names per material
				//that corresponds to the texture that was assigned to that slot in order to match safely / all cases.
				MatItr.ParameterNames[OutItr->mDesc.mIdentifier.c_str()]->Texture = Texture;
			}
		}

		//Let the material update itself if necessary
		if (MatItr.Material->IsValidLowLevel())
		{
...
void ApplyPresetData(SubstanceAir::GraphInstance* Destination, SubstanceAir::Preset& Source)
{
