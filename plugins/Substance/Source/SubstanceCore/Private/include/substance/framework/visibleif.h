...
protected:
	...
	static NodeQueue BuildEvalQueue(const SubstanceAir::InputInstanceBase* substanceInput);

	//! Evaluate queue to get VisibleIf
	static bool ProcessEvalQueue(NodeQueue& queue);

	//! Gets the current token
	static bool GetToken(const char* ptr, Node* node, int* consumedChars, bool* abort, const SubstanceAir::InputInstanceBase* substanceInput);
	...
	static SubstanceAir::InputInstanceBase* FindSubstanceInput(SubstanceAir::GraphInstance* Graph, char* Input);
...
SubstanceAir::InputInstanceBase* VisibleIf::FindSubstanceInput(SubstanceAir::GraphInstance* Graph, char* Input)
{
...
bool VisibleIf::IsCommand(const Node& node)
{
	return node.type == NodeOperator && node.operatorType <= OpTypeLeftParen;
}

void VisibleIf::ResolveSubstanceInputNode(Node* node)
{
...
int VisibleIf::GetRequiredInputComponentCount(const SubstanceAir::InputInstanceBase* input)
{
	switch (input->mDesc.mType)
	{
	case Substance_IType_Float:
...
bool VisibleIf::GetToken(const char* ptr, Node* node, int* consumedChars, bool* abort, const SubstanceAir::InputInstanceBase* baseInput)
{
	...
	{
		SubstanceAir::InputInstanceBase* substanceInput = FindSubstanceInput(&baseInput->mParentGraph, inputParam);
		...
		node->operatorType = OpTypeGetComp1;
		...
		if (sscanf(suffixPtr, ".%1[xyzw]%n", component, &suffixConsumedChars) > 0)
		{
			...
			case 'x':
				...
				break;
			case 'y':
				node->operatorType = OpTypeGetComp2;
				break;
			case 'z':
				node->operatorType = OpTypeGetComp3;
				break;
			case 'w':
				node->operatorType = OpTypeGetComp4;
				break;
			}

			// 1..4 index of the ProceduralInput vector
			int componentIndex = 1 + (int)node->operatorType - OpTypeGetComp1;
			if (componentIndex > GetRequiredInputComponentCount(substanceInput))
			{
				*abort = true;
				return false;
			}
...
NodeQueue VisibleIf::BuildEvalQueue(const SubstanceAir::InputInstanceBase* substanceInput)
{
	NodeQueue outputQueue;
	NodeStack operatorStack;
	const char* ptr = substanceInput->mDesc.mGuiVisibleIf.c_str();
	...
	int consumedChars = 0;
	bool abort = false;
	while (GetToken(ptr, &node, &consumedChars, &abort, substanceInput))
	{
		if (abort)
		{
...
bool VisibleIf::ProcessEvalQueue(NodeQueue& queue)
{
...
bool VisibleIf::EvalVisibleIf(const SubstanceAir::InputInstanceBase* substanceInput)
{
	if (substanceInput->mDesc.mGuiVisibleIf.empty())
		return true;

	NodeQueue rpnQueue = VisibleIf::BuildEvalQueue(substanceInput);
