...
bool SubstanceCache::CanReadFromCache(SubstanceAir::GraphInstance* graph)
{
#if PLATFORM_PS4
	...
	{
		auto iter = graph->getOutputs().begin();
		while (iter != graph->getOutputs().end())
		{
...
bool SubstanceCache::ReadFromCache(SubstanceAir::GraphInstance* graph)
{
#if PLATFORM_PS4
	// disable caching for PS4
	return false;
	...
	if (!reinterpret_cast<GraphInstanceData*>(graph->mUserData)->ParentGraph->ParentFactory->ShouldCacheOutput())
	{
		return false;
	}
	...
	auto Oiter = graph->getOutputs().begin();
	while (Oiter != graph->getOutputs().end())
	{
		if ((*Oiter)->mEnabled)
		{
