...
void AFogOfWarManager::Tick(float DeltaSeconds) {
	...
	if (FOWTexture && LastFOWTexture && bHasFOWTextureUpdate && bIsDoneBlending) {
		...
		TFunction< void(uint8*, const FUpdateTextureRegion2D*)> DataCleanupFunc = [](uint8 *SrcData, const FUpdateTextureRegion2D *Regions) {
			FMemory::Free((void*)Regions);
			FMemory::Free(SrcData);
		};

		LastFOWTexture->UpdateTextureRegions((int32)0, (uint32)1, textureRegions, (uint32)(4 * TextureSize), (uint32)4, (uint8*)LastFrameTextureData.GetData(), DataCleanupFunc);
		...
		FOWTexture->UpdateTextureRegions((int32)0, (uint32)1, textureRegions, (uint32)(4 * TextureSize), (uint32)4, (uint8*)TextureData.GetData(), DataCleanupFunc);
